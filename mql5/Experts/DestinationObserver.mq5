//+------------------------------------------------------------------+
//|                                           DestinationObserver.mq5 |
//|                 Observer EA for copied terminal positions/orders  |
//+------------------------------------------------------------------+
#property strict
...
input string TerminalId = "copy_1";
input string FilePrefix = "destination";
...
int OnInit()
{
   ...
   Print("DestinationObserver started. TerminalId=", TerminalId, ". Files: ", positions_file, ", ", orders_file, ", ", heartbeat_file);
...
{
   EventKillTimer();
   Print("DestinationObserver stopped.");
}
...
void ExportPositions()
{
   ...
   {
      Print("Error opening destination positions file: ", GetLastError());
      return;
   }
...
void ExportOrders()
{
   ...
   if(handle == INVALID_HANDLE)
   {
      Print("Error opening destination orders file: ", GetLastError());
      return;
   }
...
void ExportHeartbeat()
{
   ...
   if(handle == INVALID_HANDLE)
   {
      Print("Error opening destination heartbeat file: ", GetLastError());
      return;
   }
